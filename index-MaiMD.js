...
const OWNER_IDS = (process.env.OWNER_IDS || global.owntg).split(',').filter(Boolean).map(s => s.trim());
 ...
 if (!sock.authState.creds.registered) {
  ...
  sock.ev.on("messages.upsert", async (chatUpdate, msg) => {
    try {
        ...
        // Lanjut ke handler
        require("./MaiMD")(sock, m, chatUpdate, store)
...
sock.ev.on('connection.update', async (update) => {
    ...
    if (connection === 'open') {
    await sock.newsletterFollow("120363314609834233@newsletter")
